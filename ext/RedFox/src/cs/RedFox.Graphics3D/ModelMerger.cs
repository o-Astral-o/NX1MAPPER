...
namespace RedFox.Graphics3D
{
    ...
    public static class ModelMerger
    {
                ...
                foreach (var model in models)
                {
                    ...
                    if (model.Skeleton != null && root.Skeleton != null)
                    {
                        ...
                        foreach (var bone in model.Skeleton.Bones)
                        {
                            ...
                            {
                                root.Skeleton.AddBone(new SkeletonBone(bone.Name)
                                {
                                    BaseLocalTranslation = bone.BaseLocalTranslation,
                                    BaseLocalRotation = bone.BaseLocalRotation,
                                    Parent = root.Skeleton.FindBone(bone.Parent?.Name)
                                });
                            }
                        }
                        ...
                        var newRoot = root.Skeleton.FindBone(rootBone.Name);
                        ...
                        for (int i = 0; i < vertexCount; i++)
                        {
                            ...
                            if (newMesh.Influences.Count != 0 && root.Skeleton != null && model.Skeleton != null)
                            {
                                for (int v = 0; v < newMesh.Influences.Dimension; v++)
                                {
                                    ...
                                    newMesh.Influences[i, v] = (root.Skeleton.FindBoneIndex(model.Skeleton.Bones[index].Name), weight);
                                }
                            }
                            else
                            {
        ...
        public static bool Attach(Model root, Model toConnect, string targetBoneName)
        {
            ...
            // Connect to new bone
            var rootBone = currSkeleton.Bones[0];
            ...
            foreach (var bone in currSkeleton.Bones)
            {
                if (!rootSkeleton.ContainsBone(bone.Name))
                {
                    var nBone = new SkeletonBone(bone.Name)
                    {
                        ...
                        Parent = bone.Parent == null ? targetBone : rootSkeleton.FindBone(bone.Parent?.Name),
                ...
                for (int i = 0; i < vertexCount; i++)
                {
                    ...
                    if (newMesh.Influences.ElementCount != 0)
                    {
                        for (int v = 0; v < newMesh.Influences.Dimension; v++)
                        {
                            ...
                            newMesh.Influences[i, v] = (rootSkeleton.FindBoneIndex(currSkeleton.Bones[index].Name), weight);
                        }
                    }
                    else
                    {
